...
object BotBuilder {
    ...
    fun createCustomBot(token: String, intents: MutableList<GateWayIntent>): BotBuilder {
        BotBuilder.token = token
        BotBuilder.intents.addAll(intents)
        return this
    }
    ...
    fun setOkHttpClient(okHttpClient: OkHttpClient): BotBuilder {
        BotBuilder.okHttpClient = okHttpClient
        return this
    }
    ...
    fun setAllowedCache(allowedCache: MutableList<CacheIds>, clearCache: Boolean): BotBuilder {
        if (clearCache) {
            BotBuilder.allowedCache.clear()
        }
        BotBuilder.allowedCache.addAll(allowedCache)
        return this
    }
    ...
    fun setDisallowedCache(
        ...
        if (clearCache) {
            ...
            BotBuilder.disallowedCache.clear()
        }
        BotBuilder.disallowedCache.addAll(disallowedCache)
        return this
    }
    ...
    fun setGuildIds(guildIds: MutableList<String>): BotBuilder {
        BotBuilder.guildIds.addAll(guildIds)
        return this
    }
    ...
    fun setUserStatus(userStatus: UserStatus): BotBuilder {
        BotBuilder.userStatus = userStatus
        return this
    }
    ...
    fun setETFInsteadOfJson(etfInsteadOfJson: Boolean): BotBuilder {
        BotBuilder.etfInsteadOfJson = etfInsteadOfJson
        return this
    }
    ...
    fun setEnableShutDownHook(enableShutDownHook: Boolean): BotBuilder {
        BotBuilder.enableShutDownHook = enableShutDownHook
        return this
    }
     ...
     */
    fun build(): YDWK {
        val ydwk: YDWKImpl = YDWKImpl(okHttpClient ?: OkHttpClient())
