...
interface Guild : SnowFlake, NameAbleEntity, GenericEntity {
  ...
  /**
   * The maximum number of presences for the guild (null is always returned, apart from the largest
   * of guilds).
   *
   ...
   * @return A [RestResult] that completes when the ban is created.
   */
  suspend fun banUser(userId: Long, reason: String? = null) = banUser(userId, Duration.ZERO, reason)
  ...
  val everyoneRole: Role
    get() =
      getRoleById(idAsLong) ?: throw IllegalStateException("The @everyone role is not present.")
}
