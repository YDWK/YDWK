...
class GuildUpdateHandlerExtended(ydwk: YDWKImpl, json: JsonNode) : GuildUpdateHandler(ydwk, json) {
  ...
  private fun onAfkChannelChange(guild: GuildImpl, oldAfkChannelId: Long?, newAfkChannelId: Long?) {
  ...
  private fun onMaxPresencesChange(guild: GuildImpl, oldMaxPresences: Int?, newMaxPresences: Int?) {
  ...
  private fun onStickersChange(
    ...
    guild.stickers =
      newStickers?.let { it -> it.map { StickerImpl(ydwk, it, it["id"].asLong()) } } ?: emptyList()
    ydwk.emitEvent(
