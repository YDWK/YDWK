...
import io.github.ydwk.ydwk.impl.entities.ApplicationImpl
import io.github.ydwk.ydwk.impl.entities.UserImpl
import io.github.ydwk.ydwk.util.EntityToStringBuilder
import java.time.ZonedDateTime

class InviteImpl(override val ydwk: YDWK, override val json: JsonNode) : Invite {
    override val code: String
        get() = json["code"].asText()

    override val guild: Guild
        get() =
            ydwk.getGuildById(json["guild"]["id"].asText())
                ?: throw IllegalStateException("Guild is null")

    override val channel: GuildChannel
        get() =
            ydwk.getGuildChannelById(json["channel"]["id"].asText())
                ?: throw IllegalStateException("Channel is null")

    override val inviter: User?
        get() =
            if (json["inviter"] != null)
                UserImpl(json["inviter"], json["inviter"]["id"].asLong(), ydwk)
            else null
    ...
    override val targetUser: User?
        get() =
            if (json["target_user"] != null)
                UserImpl(json["target_user"], json["target_user"]["id"].asLong(), ydwk)
            else null
    ...
    override val targetApplication: Application?
        get() =
            if (json["target_application"] != null)
                ApplicationImpl(
                    json["target_application"], json["target_application"]["id"].asLong(), ydwk)
            else null
    ...
    override val guildScheduledEvent: GuildScheduledEvent
        get() =
            GuildScheduledEventImpl(
                ydwk, json["guild_scheduled_event"], json["guild_scheduled_event"]["id"].asLong())
