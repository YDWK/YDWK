...
class GuildImpl(override val ydwk: YDWK, override val json: JsonNode, override val idAsLong: Long) :
  ...
  override var isOwner: Boolean? = if (json.hasNonNull("owner")) json["owner"].asBoolean() else null
  ...
  override var widgetChannelId: GetterSnowFlake? =
    if (json.hasNonNull("widget_channel_id")) GetterSnowFlake.of(json["widget_channel_id"].asLong())
    else null
  ...
  override var systemChannelId: GetterSnowFlake? =
    if (json.hasNonNull("system_channel_id")) GetterSnowFlake.of(json["system_channel_id"].asLong())
    else null
  ...
  override var rulesChannelId: GetterSnowFlake? =
    if (json.hasNonNull("rules_channel_id")) GetterSnowFlake.of(json["rules_channel_id"].asLong())
    else null
  ...
  override var approximatePresenceCount: Int? =
    if (json.hasNonNull("approximate_presence_count")) json["approximate_presence_count"].asInt()
    else null

  override var welcomeScreen: WelcomeScreen? =
    if (json.hasNonNull("welcome_screen")) WelcomeScreenImpl(ydwk, json["welcome_screen"]) else null
  ...
  override var isBoostProgressBarEnabled: Boolean = json["premium_progress_bar_enabled"].asBoolean()
  ...
  override val requestBans: CompletableFuture<List<Ban>>
    get() {
      return ydwk.restApiManager.get(EndPoint.GuildEndpoint.GET_BANS, id).execute { it ->
        ...
        jsonBody?.map { BanImpl(ydwk, it) } ?: throw IllegalStateException("Response body is null")
      }
    }
