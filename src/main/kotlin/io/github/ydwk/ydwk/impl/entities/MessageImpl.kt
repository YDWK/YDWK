...
class MessageImpl(
  ...
  override val editedTime: String?
    get() =
      if (json.has("edited_timestamp")) formatZonedDateTime(json.get("edited_timestamp").asText())
      else null
  ...
  override val mentionedRoles: List<Role>
    get() {
      ...
      json.get("mention_roles").forEach { list.add(RoleImpl(ydwk, it, it.get("id").asLong())) }
      return list
    }
  ...
  override val attachments: List<Attachment>
    get() {
      ...
      json.get("attachments").forEach { list.add(AttachmentImpl(ydwk, it, it.get("id").asLong())) }
      return list
    }
  ...
  override val webhookId: GetterSnowFlake?
    get() =
      if (json.has("webhook_id")) GetterSnowFlake.of(json.get("webhook_id").asLong()) else null
    ...
    get() =
      if (json.has("application"))
        PartialApplicationImpl(
          ...
          json.get("application").get("id").asLong(),
          ydwk
        )
      else null
  ...
  override val messageReference: MessageReference?
    get() =
      if (json.has("message_reference")) MessageReferenceImpl(ydwk, json.get("message_reference"))
      else null
    ...
    get() =
      if (json.has("interaction"))
        MessageInteractionImpl(
          ydwk,
          json.get("interaction"),
          json.get("interaction").get("id").asLong()
        )
      else null
  ...
  override val thread: GenericGuildChannel?
    get() =
      if (json.has("thread"))
        GenericGuildChannelImpl(ydwk, json.get("thread"), json.get("thread").get("id").asLong())
      else null
