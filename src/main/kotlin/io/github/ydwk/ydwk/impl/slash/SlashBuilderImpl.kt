...
class SlashBuilderImpl(
  ...
  override fun build() {
          ...
          .execute(
            fun(it: CompletableFutureManager): Map<MutableMap<Long, Long>, String> {
              ...
              return jsonBody?.associate {
                mutableMapOf(it["guild_id"].asLong() to it["id"].asLong()) to it["name"].asText()
              }
                ?: emptyMap()
            }
          )
    ...
    for (slash in slashCommands) {
      ...
      Checks.checkLength(slash.name, 32, "Slash command name can not be longer than 32 characters")
      Checks.checkLength(
        slash.description,
        100,
    ...
    for (slash in specificGuildSlash) {
      if (currentSpecificGuildCommandIdAndNameMap.containsValue(slash.name)) {
        ydwk.logger.debug("Specific guild slash command ${slash.name} already exists, updating...")
  ...
  private fun addGuildSlashCommand(rest: RestApiManager, guildId: String, json: RequestBody) {
    rest
      .post(json, EndPoint.ApplicationCommandsEndpoint.CREATE_GUILD_COMMAND, applicationId, guildId)
      .execute()
  }
