...
fun checkForDataClasses() {
  if (File("src/main/kotlin/io/github/ydwk/ydwk/evm/backend/event/GenericEvent.kt").exists()) {
    ...
    if (files.isNullOrEmpty()) {
      ...
      for (file in files) {
        val folder = File("src/main/kotlin/io/github/ydwk/ydwk/evm/event/events/" + file.name)
        if (folder.exists()) {
  ...
  for (subFiles in folder.listFiles()!!) {
    ...
    if (subFiles.isDirectory) {
      ...
      if (!dataClass && !openClass) {
        throw Exception("The class $className is not a data class. Please make it a data class")
      }
    }
  }
}
