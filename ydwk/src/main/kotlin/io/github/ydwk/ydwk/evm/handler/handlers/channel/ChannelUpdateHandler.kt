...
class ChannelUpdateHandler(ydwk: YDWKImpl, json: JsonNode) : Handler(ydwk, json) {
  ...
  private fun updateTextChannel() {
    ...
    if (channel == null) {
      ...
      ydwk.cache[json["id"].asText(), ydwk.entityInstanceBuilder.buildGuildTextChannel(json)] =
        CacheIds.CHANNEL
      return
    }
    ...
    if (oldSlowMode != newSlowMode) {
      ...
      ydwk.emitEvent(
        TextChannelSlowModeUpdateEvent(ydwk, channel, channel.type, oldSlowMode, newSlowMode)
      )
    }
  }
  ...
  private fun updateNewsChannel() {
    ...
    if (channel == null) {
      ydwk.logger.info("Channel ${json["id"].asText()} is not cached, creating new one.")
      ydwk.cache[json["id"].asText(), ydwk.entityInstanceBuilder.buildGuildNewsChannel(json)] =
        CacheIds.CHANNEL
      return
    }
  ...
  private fun updateMessageChannel(channel: GuildMessageChannel) {
    ...
    if (!Objects.deepEquals(oldLastMessageId, newLastMessageId)) {
      ...
      ydwk.emitEvent(
        MessageChannelLastMessageIdUpdateEvent(ydwk, channel, oldLastMessageId, newLastMessageId)
      )
    }
    ...
    val newPermissionOverwrites =
      json["permission_overwrites"].map { ydwk.entityInstanceBuilder.buildPermissionOverwrite(it) }
  ...
  private fun updateGuildChannel(channel: GuildChannel) {
    ...
    if (oldPosition != newPosition) {
      ...
      ydwk.emitEvent(
        GuildChannelPositionUpdateEvent(ydwk, channel, channel.type, oldPosition, newPosition)
      )
    }
    ...
    if (!Objects.deepEquals(oldParentId, newParentId)) {
      ...
      ydwk.emitEvent(
        GuildChannelParentUpdateEvent(ydwk, channel, channel.type, oldParentId, newParentId)
      )
    }
  }
  ...
  private fun updateStageChannel() {
    ...
    if (channel == null) {
      ydwk.logger.info("Channel ${json["id"].asText()} is not cached, creating new one.")
      ydwk.cache[json["id"].asText(), ydwk.entityInstanceBuilder.buildGuildStageChannel(json)] =
        CacheIds.CHANNEL
      return
    }
  ...
  private fun updateVoiceChannel() {
    ...
    if (channel == null) {
      ydwk.logger.info("Channel ${json["id"].asText()} is not cached, creating new one.")
      ydwk.cache[json["id"].asText(), ydwk.entityInstanceBuilder.buildGuildVoiceChannel(json)] =
        CacheIds.CHANNEL
      return
    }
  ...
  private fun updateVoiceChannel(channel: GuildVoiceChannel) {
    ...
    if (oldUserLimit != newUserLimit) {
      ...
      ydwk.emitEvent(VoiceChannelUserLimitUpdateEvent(ydwk, channel, oldUserLimit, newUserLimit))
    }
  ...
  private fun updateForumChannel() {
    ...
    if (channel == null) {
      ydwk.logger.info("Channel ${json["id"].asText()} is not cached, creating new one.")
      ydwk.cache[json["id"].asText(), ydwk.entityInstanceBuilder.buildGuildForumChannel(json)] =
        CacheIds.CHANNEL
      return
    }
    ...
    if (!Objects.deepEquals(oldAvailableForumTags, newAvailableForumTags)) {
      ...
      ydwk.emitEvent(
        ForumChannelAvailableForumTagsUpdateEvent(
          ydwk,
          channel,
          oldAvailableForumTags,
          newAvailableForumTags,
        )
      )
    }
    ...
    val newDefaultForumLayoutView = ForumLayoutType.getValue(json["default_forum_layout"].asInt())
