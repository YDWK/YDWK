...
class UserUpdateHandler(ydwk: YDWKImpl, json: JsonNode) : Handler(ydwk, json) {
  ...
  override suspend fun start() {
    ...
    val newBanner: String? = if (userJson.has("banner")) userJson.get("banner").asText() else null
    ...
    val newLocale: String = if (userJson.has("locale")) userJson.get("locale").asText() else "en-US"
    ...
    if (!Objects.deepEquals(oldVerified, newVerified)) {
      ...
      ydwk.emitEvent(UserVerifiedUpdateEvent(ydwk, user, oldVerified.let { false }, newVerified))
    }
