...
class VoiceWebSocket(
   ...
   * @throws IllegalStateException if the voice server payload is null, the voice state is null, the
   *   voice endpoint is null, the session ID is blank or null, or the token is blank or null.
  ...
  private fun identify() {
    ...
    identifyData.put("server_id", voiceConnection.voiceChannel.guildId.asLong)
  ...
  private fun resume() {
    ...
    resumeData.put("server_id", voiceConnection.voiceChannel.guildId.asLong)
   ...
   *    WebSocket to initiate the process of receiving and sending data. This is done using Opcode 1
   *    Select Protocol.
   *
  ...
  private fun handleProtocol(modes: MutableList<String>) {
    try {
      ...
      selectProtocolDataJson.put("mode", VoiceEncryption.getPreferred(modes).getPreferredMode())
  ...
  override fun onDisconnected(
    ...
    val closeCodeAsString: String =
      if (closeFrame != null)
        VoiceCloseCode.fromCode(closeFrame.closeCode).name + " (" + closeFrame.closeCode + ")"
