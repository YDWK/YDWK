...
class GuildUpdateHandlerExtended(ydwk: YDWKImpl, json: JsonNode) : GuildUpdateHandler(ydwk, json) {
  ...
  private fun onDiscoverySplashChange(
    ...
    logger.debug("Guild discovery splash changed from $oldDiscoverySplash to $newDiscoverySplash")
  }
  ...
  private fun onPermissionsChange(guild: Guild, oldPermissions: String?, newPermissions: String?) {
  ...
  private fun onAfkChannelChange(guild: Guild, oldAfkChannelId: Long?, newAfkChannelId: Long?) {
  ...
  private fun onWidgetEnabledChange(
    ...
    ydwk.emitEvent(GuildWidgetEnabledUpdateEvent(ydwk, guild, wasWidgetEnabled, isWidgetEnabled))
  ...
  private fun onExplicitContentFilterChange(
    ...
    guild.explicitContentFilterLevel = ExplicitContentFilterLevel.getValue(newExplicitContentFilter)
    ydwk.emitEvent(
  ...
  private fun onMfaLevelChange(guild: Guild, oldMfaLevel: Int, newMfaLevel: Int) {
    ...
    ydwk.emitEvent(
      GuildMfaLevelUpdateEvent(
        ydwk,
        guild,
        MFALevel.getValue(oldMfaLevel),
        MFALevel.getValue(newMfaLevel),
      )
    )
  ...
  private fun onApplicationIdChange(
    ...
    ydwk.emitEvent(GuildApplicationIdUpdateEvent(ydwk, guild, oldApplicationId, newApplicationId))
  ...
  private fun onRulesChannelChange(
    ...
    ydwk.emitEvent(GuildRulesChannelUpdateEvent(ydwk, guild, oldRulesChannelId, newRulesChannelId))
    logger.debug("Guild rules channel changed from $oldRulesChannelId to $newRulesChannelId")
  }

  private fun onMaxPresencesChange(guild: Guild, oldMaxPresences: Int?, newMaxPresences: Int?) {
  ...
  private fun onDescriptionChange(guild: Guild, oldDescription: String?, newDescription: String?) {
  ...
  private fun onPreferredLocaleChange(
    ...
    logger.debug("Guild preferred locale changed from $oldPreferredLocale to $newPreferredLocale")
  }
  ...
  private fun onStickersChange(guild: Guild, oldStickers: List<Sticker>, newStickers: JsonNode?) {
    guild.stickers =
  ...
  private fun onEmojiChange(guild: Guild, oldEmojis: List<Emoji>, newEmojis: JsonNode?) {
    ...
    ydwk.emitEvent(
      GuildEmojisUpdateEvent(
        ...
        guild,
        oldEmojis,
        newEmojis?.let { it -> it.map { ydwk.entityInstanceBuilder.buildEmoji(it) } } ?: emptyList(),
      )
    )
  ...
  private fun onFeaturesChange(
    ...
    guild.features =
      newFeatures?.let { it -> it.map { GuildFeature.getValue(it.asText()) }.toSet() } ?: emptySet()
    ydwk.emitEvent(
