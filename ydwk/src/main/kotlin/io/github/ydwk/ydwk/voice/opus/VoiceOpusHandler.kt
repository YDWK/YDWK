 ...
 *
 * Expects raw 16-bit signed little-endian stereo PCM at 48 kHz. Each call to [encode] must receive
 * exactly [PCM_FRAME_BYTES] bytes (20 ms of audio).
 */
class VoiceOpusHandler {
  ...
  fun encode(pcm: ByteArray): ByteArray {
    ...
    require(pcm.size == PCM_FRAME_BYTES) { "Expected $PCM_FRAME_BYTES PCM bytes, got ${pcm.size}" }
